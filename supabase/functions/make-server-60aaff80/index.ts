...
app.put('/make-server-60aaff80/complaints/:id/resolve-state', async (c) => {
  try {
    ...
    if (complaintError) {
      ...
      return c.json({ error: 'Failed to resolve complaint', details: complaintError.message }, 500);
    }
...
app.post('/make-server-60aaff80/ai/dashboard-chat', async (c) => {
  try {
    ...
    if (!scope || !question || !stateId) {
      return c.json({ error: 'scope, question and stateId are required.' }, 400);
    }

    if (scope === 'municipal' && !municipalId) {
      return c.json({ error: 'municipalId is required for municipal scope.' }, 400);
    }
    ...
    if (categoriesError) {
      return c.json({ error: `Failed to fetch categories: ${categoriesError.message}` }, 500);
    }
    ...
    if (departmentsError) {
      return c.json({ error: `Failed to fetch departments: ${departmentsError.message}` }, 500);
    }

    const deptMap = new Map((departments || []).map((d: any) => [d.id, d.icon]));
    ...
    if (scope === 'municipal') {
      ...
      const total = complaints?.length || 0;
      const pending = complaints?.filter((x: any) => x.status === 'pending').length || 0;
      const verified = complaints?.filter((x: any) => x.status === 'verified').length || 0;
      const resolved = complaints?.filter((x: any) => x.status === 'resolved').length || 0;

      const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
      const monthlyMap: any = {};
      (complaints || []).forEach((item: any) => {
        const month = monthNames[new Date(item.submitted_date).getMonth()];
      ...
      if (municipalsError) {
        return c.json({ error: `Failed to fetch municipals: ${municipalsError.message}` }, 500);
      }

      const municipalIds = (municipals || []).map((m: any) => m.id);
      const { data: complaints, error: complaintsError } = await supabase
        .from('complaints_with_verification_count')
        .select('*')
      ...
      const now = Date.now();
      const escalatedComplaints = (complaints || []).filter((x: any) => {
        if (x.status !== 'pending') return false;
        const days = Math.floor((now - new Date(x.submitted_date).getTime()) / (1000 * 60 * 60 * 24));
        return days >= 7;
      });
    ...
    const compactHistory = (chatHistory || [])
      .slice(-8)
      .map((item: any) => `${item.role === 'assistant' ? 'Assistant' : 'User'}: ${item.text || ''}`)
      .join('\n');

    const prompt = `
You are CivicChain's governance assistant.
Answer only from the dashboard JSON context below.
If data is missing, clearly say it is not available in current dashboard data.
